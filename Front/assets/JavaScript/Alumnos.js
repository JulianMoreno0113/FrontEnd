...
function Post() {
    fetch(url, {
        ...
        swal("Transacción ejecutada con éxito", "Se ha agregado correctamente al estudiante", "success");
                Get();
                LimpiarTextBox();
    }).catch(err =>
        swal({
            title: "Error al ejecutar la transacción",
            text: "Ha ocurrido un error en la transacción por favor comunicate con Sincosoft por medio de un help desk error: " + + err.message,
            icon: "error",
            button: "Ok!",
        })
    );
